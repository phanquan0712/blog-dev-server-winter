...
const commentCtrl = {
   ...
   updateComment: async (req: IReqAuth, res: Response) => {
      ...
      try {
         ...
         if(!comment) return res.status(404).json({ msg: 'Comment does not exits.' })
   ...
   deleteComment: async (req: IReqAuth, res: Response) => {
      ...
      try {
         const comment = await Comments.findOneAndDelete({ 
            _id: req.params.id,
            $or: [ // One of them, it is user or blog_user_id
               { user: req.user.id },
         ...
         if(comment.comment_root) {
            ...
            await Comments.updateMany({ _id: comment.comment_root }, {
