...
const commentCtrl = {
            ...
            const data = yield commentModel_1.default.aggregate([
                {
                    $facet: {
                        totalData: [
                            {
                                $match: {
                                    blog_id: mongoose_1.default.Types.ObjectId(req.params.id),
    ...
    replyComment: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        if (!req.user)
            return res.status(400).json({ msg: 'Invalid Authentication' });
        try {
            const { content, blog_id, blog_user_id, reply_user, comment_root } = req.body;
            const newComment = new commentModel_1.default({
                user: req.user._id,
                content,
                blog_id,
    ...
    updateComment: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            const { data } = req.body;
            const comment = yield commentModel_1.default.findByIdAndUpdate({
                // Find
                _id: req.params.id,
                user: req.user.id
            }, {
    ...
    deleteComment: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            const comment = yield commentModel_1.default.findOneAndDelete({
                ...
                $or: [
                    { user: req.user.id },
                    { blog_user_id: req.user.id }
                ]
            });
            if (!comment)
                return res.status(404).json({ msg: 'Comment does not exits.' });
            if (comment.comment_root) {
                // Delete one comment in replyCM, Update replyCM, after delete comment
                yield commentModel_1.default.updateMany({ _id: comment.comment_root }, {
                    $pull: { replyCM: comment._id }
                });
            }
            else {
                // Delete comment root, should delete all replyCM
